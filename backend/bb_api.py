...
# Make the standalone bugbounty_tool package importable
...
# In-memory job registry. Each entry holds live progress for SSE / polling.
JOBS: Dict[str, Dict] = {}
...
def _run_job(job_id: str, payload: ScanRequest, db: AsyncIOMotorDatabase) -> None:
    ...
    try:
        ...
        try:
            ...
            # Persistence is best-effort; do not crash the worker.
            pass
...
# ---------------------------------------------------------------------------
# Factory: build the router with a DB handle injected
# ---------------------------------------------------------------------------
...
def build_router(db: AsyncIOMotorDatabase) -> APIRouter:
    ...
    async def health() -> Dict[str, str]:
        return {"status": "ok", "service": "bugbounty-toolkit"}
    ...
    async def create_scan(payload: ScanRequest) -> JobSummary:
        ...
        allowed = {"recon", "scan", "vuln"}
        bad = [m for m in payload.modules if m not in allowed]
        if bad:
            raise HTTPException(status_code=400,
                                detail=f"Unknown module(s): {bad}")
        ...
        thread = threading.Thread(
            target=_run_job, args=(job_id, payload, db), daemon=True
        )
        thread.start()
        return JobSummary(
            id=job_id,
    ...
    async def list_scans(limit: int = 50) -> List[Dict]:
        ...
        # De-duplicate (in-memory take precedence)
